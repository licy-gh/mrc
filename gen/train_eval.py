...
tmp_state_dict['bert.embeddings.word_embeddings.weight'] = torch.index_select(
    tmp_state_dict['bert.embeddings.word_embeddings.weight'],
    0,
    torch.tensor(keep_tokens, dtype=torch.long)
)
...
model = BojoneModel.from_pretrained(
...
optimizer, scheduler = create_optimizer_and_scheduler(
    model,
    lr=learning_rate,
    num_training_steps=train_generator.steps * epochs
)
...
class ReadingComprehension(AutoRegressiveDecoder):
    ...
    def __init__(self, mode='generative', **kwargs):
    ...
    def answer(self, question, passages, topk=1):
        ...
        for passage in passages:
            passage = re.sub(u' |、|；|，', ',', passage)
            p_token_ids = tokenizer.encode(passage, maxlen=gen_max_p_len)[0]
            q_token_ids = tokenizer.encode(question, maxlen=gen_max_q_len + 1)[0]
...
for e in range(epochs):
    ...
    for step, batch in enumerate(train_generator):
        ...
        if (step + 1) % gradient_accumulation_steps == 0:
            torch.nn.utils.clip_grad_norm_(model.parameters(), gen_max_grad_norm)
            optimizer.step()
            scheduler.step()
            optimizer.zero_grad()
    ...
    # evaluation
    final = sum(list(eval_valid_data(valid_data[:1000], e)))
    if final > best_final:
        best_final = final
